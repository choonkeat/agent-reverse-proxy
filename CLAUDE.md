...
After any change to `cmd/example/` (main.go or content files), run `make example-run EXAMPLE_PORT=3003` to verify all 8 Playwright steps pass. This is the end-to-end conformance test for the proxy.
...
2. `make example-run EXAMPLE_PORT=3003` — all 8 example app steps must pass (if `cmd/example/` was touched)
...
- Steps with server-side logic: step 5 (set cookie), step 6 (read cookie, two HTML variants), step 7 (POST + redirect), step 8 (template with `{{COOKIE_STATUS}}`)
