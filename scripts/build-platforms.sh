...
echo "Building agent-reverse-proxy v${VERSION}"
...
for target in "${TARGETS[@]}"; do
  ...
  bin_name="agent-reverse-proxy"
  if [ "$goos" = "windows" ]; then
    bin_name="agent-reverse-proxy.exe"
  fi
...
{
  "name": "@choonkeat/agent-reverse-proxy-${pkg_suffix}",
  "version": "${VERSION}",
  "description": "agent-reverse-proxy binary for ${goos}/${goarch}",
  "license": "MIT",
  "os": ["${npm_os}"],
  "cpu": ["${npm_cpu}"],
  "files": ["bin/"]
